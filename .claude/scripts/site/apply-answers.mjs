...
// возвращается РОВНО В ОДНО поле. Список решений один на два файла и лежит в _contract.mjs:
...
import { DECISIONS, readDecision, writeDecision, today, isCheckable } from "./_contract.mjs";
...
const answers = new Map();   // key -> { value, from }
const unknown = [];
for (const raw of lines) {
  ...
  if (!/^(d[1-8]|f\d{2,3}|g\d{1,2})$/.test(key)) { unknown.push(`${key}: ${value}`); continue; }
  answers.set(key, { value, from });
}
