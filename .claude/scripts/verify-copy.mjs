#!/usr/bin/env node
// verify-copy.mjs
// Pre-flight КОПИ-валидатор (v4): механические пункты (1-13 + 14а тире) чек-листа COPY-AUDIT.md (14 пунктов) по page.json
// ДО сборки HTML. Семантические пункты (за счёт / H2-выгода / OPSEC / п.14б-д ИИ-маркеры) добивает агент copy-auditor.
//
...
const jarg = (low.match(/(?<![а-яёa-z0-9_])(customer dev|jtbd|cjm|оффер|конверси[а-яё]*|воронк[а-яё]*|сегмент[а-яё]*|персонаж|аватар|утп|usp|лид-?маг[а-яё]*)(?![а-яёa-z0-9_])/gi) || []);
...
// 14а. длинное/среднее тире - маркер ИИ номер один (COPY-AUDIT.md п.14а), жёсткое правило проекта
const dashCount = (allText.match(/—|–/g) || []).length + (allText.includes(h1) ? 0 : (h1.match(/—|–/g) || []).length);
if (dashCount) V(`п.14а длинное/среднее тире: ${dashCount} шт - заменить на дефис (-)`);
// 14б. слова-маркеры ИИ (W - семантику добивает copy-auditor); lookaround вместо \b (в JS \b ASCII-only)
if (/(?<![а-яё])(важно отметить|стоит отметить|следует подчеркнуть|таким образом|подводя итог|на сегодняшний день|в современном мире|не секрет, что)(?![а-яё])/i.test(low)) W("п.14б слова-маркеры ИИ - переформулировать");
// бюджет блоков
