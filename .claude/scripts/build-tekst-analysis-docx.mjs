...
const decLabels = { positioning: "Позиционирование (одной строкой)", idea: "Идея / красная нить (метафора)", price_presentation: "Подача цены", cta_tone: "Формулировка главной кнопки (CTA)" };
if (Object.keys(decisions).length) {
  ...
  for (const [key, label] of Object.entries(decLabels)) {
    ...
    d.variants.forEach((v, i) => {
      ...
      const runs = [new TextRun({ text: `${i + 1}. ${v}${rec}`, bold: i === d.recommended, font: "Arial", size: 22 })];
      if (d.region_flag && d.region_flag[i]) runs.push(new TextRun({ text: "  [звучит «по-западному» - для региона риск]", italics: true, color: "C00000", font: "Arial", size: 18 }));
