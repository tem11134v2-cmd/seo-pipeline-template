...
// Исключение - legal.phone_absent: заказчик решил, что телефонного канала нет вовсе
// (онлайн-проект). Тогда требовать телефон значит требовать выдумать его; проверка
// переворачивается: телефона в документе быть НЕ должно.
const phoneAbsent = Boolean(mainManifest.legal && mainManifest.legal.phone_absent);
if (phoneAbsent) {
  if (/href="tel:\+?\d/.test(html))
    V('legal.phone_absent, но в документе есть tel-ссылка - решение заказчика «телефона нет» не применилось (пересобери ассемблером)');
} else if (!/href="tel:\+?\d/.test(html)) {
  V('нет кликабельного телефона (tel: пуст или отсутствует). Телефона нет у заказчика вовсе - это решение, а не пустое поле: legal.phone_absent: true');
}
...
// Неподставленный маркер кита. Латинские заглавные в комментарии - это ВСЕГДА маркер shell
// или фрагмента (русские служебные комментарии под шаблон не попадают). Такой остаток
// означает, что ассемблер не знает про маркер: в документе он невидим, но блок, который
// маркер нес (меню, надпись кнопки, график), просто не появился - и заметить это можно
// только глазами на готовом файле.
{
  const left = [...new Set([...html.matchAll(/<!--\/?(?:IF_)?([A-Z][A-Z_]*)-->/g)].map((m) => m[0]))];
  if (left.length) V(`в собранном документе остались маркеры кита: ${left.slice(0, 6).join(", ")}${left.length > 6 ? ` и еще ${left.length - 6}` : ""} - ассемблер их не подставил (кит новее скрипта?)`);
}
...
if (legalManifest) {
  ...
  // ИНН и ОГРН печатаются в подвал живого сайта и уезжают верстальщику. До этой проверки
  // формат реквизита не смотрел НИКТО: на боевом прогоне в подвал уехал бы ИНН, склеенный
  // из ИНН, даты подтверждения и номера вопроса (21 цифра), и поймала это только ручная
  // вычитка по буквам. Проверяем ТОЛЬКО строку из чистых цифр: пустое поле и пометка
  // «[ИНН - требует уточнения]» - законные состояния незакрытого реквизита, их закрывает
  // заказчик. Снять проверку на спорном, но верном числе можно полем legal.<поле>_verified.
  const REQ_SPEC = { inn: { lens: [10, 12], name: "ИНН" }, ogrn: { lens: [13, 15], name: "ОГРН" } };
  for (const field of ["inn", "ogrn"]) {
    const raw = String(legalManifest[field] || "").trim();
    if (!raw || !/^\d+$/.test(raw)) continue;
    if (legalManifest[`${field}_verified`] === true) continue;
    const { lens: want, name } = REQ_SPEC[field];
    if (!want.includes(raw.length)) {
      V(`legal.${field}: ${raw.length} цифр (${name} бывает ${want.join(" или ")}) - похоже на склейку реквизита с соседним числом. Проверь facts.json и inputs.json, реквизит закрывает заказчик`);
      continue;
    }
    if (field === "inn" && !innChecksumOk(raw))
      V(`legal.inn: контрольная цифра ИНН не сходится (${raw}) - в реквизите опечатка или перестановка. Сверь по документу заказчика; если число верное, пометь legal.inn_verified: true`);
  }
}

// Контрольная цифра ИНН (10 и 12 знаков) - стандартный алгоритм ФНС. Нужен именно он:
// длина ловит склейку, но не ловит опечатку внутри правильного количества цифр, а такая
// опечатка в подвале сайта неотличима на глаз от настоящего реквизита.
function innChecksumOk(inn) {
  const d = inn.split("").map(Number);
  const dot = (w) => w.reduce((a, k, i) => a + k * d[i], 0) % 11 % 10;
  if (d.length === 10) return dot([2, 4, 10, 3, 5, 9, 4, 6, 8]) === d[9];
  if (d.length === 12) {
    return dot([7, 2, 4, 10, 3, 5, 9, 4, 6, 8]) === d[10]
      && dot([3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8]) === d[11];
  }
  return true;
}
