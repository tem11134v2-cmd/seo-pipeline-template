...
- [ ] Набор и порядок строк `card` - ровно 22 пункта из §5.5 (Домен … Ссылки доменов-доноров). Состав и порядок этих меток машинно энфорсит `verify-audit.mjs` по эталону `CARD_LABELS` (blocking, exit 2) - расхождение в метке/порядке валит гейт.
