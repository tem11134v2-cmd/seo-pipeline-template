...
4. Если среди кандидатов есть с `commercial_pct >= порог OK` для этого типа страницы - **заменить** маркер:
...
5. Если **ни один** синоним не прошёл - оставить исходный маркер, поставить:
   - `commerce_warning = "ТОП-10 преимущественно info-сайты (commercial_pct: <X>%). Рекомендуется: 1) сменить тип страницы на article и переместить в блог, 2) объединить с соседней коммерческой страницей, 3) исключить из работы. Решение - за клиентом."`
   - `commerce_note = "info_dominant"`

Эта пометка **сохраняется до xlsx** (в колонку «Примечания») и **используется `semantic-expander`** (он пропустит страницу при расширении JM, чтобы не тратить лимиты).
...
```json
{
  ...
  "commerce_warning": null | "<строка для клиента>",
...
```json
{
  ...
  "pages": [
    ...
    },
    {
      ...
      "commercial_pct": 20,
      "commerce_note": "info_dominant",
      "commerce_warning": "ТОП-10 преимущественно info-сайты (commercial_pct: 20%). Рекомендуется: 1) сменить тип страницы на article и переместить в блог, 2) объединить с соседней коммерческой страницей, 3) исключить из работы. Решение - за клиентом.",
      "original_marker": null,
...
- ⚠️ **info-dominant маркеры** (страницы, выдача которых не подходит для коммерческой посадки): `<список «название_страницы (X%)»>` - **будут пропущены при JM-расширении**, клиент должен решить: убрать страницу / сделать article / объединить
