...
MCP-серверы подключены глобально. Используемые инструменты: `wk_check_frequency`, `arsenkin_commerce`, `jm_suggest`, `jm_account`. (Дерево регионов Wordstat живым инструментом не отдаётся - код региона берём из зашитого списка / дефолт 213, см. region-guard ниже.)
...
`region = inputs.region_yandex` (число). Если это country-код (`225`, `0`, `null`) или регион не определён - подставь `213` (Москва) и запиши `region_note` в research.json. Город не из стандартного списка (PLAYBOOK р.8) - возьми код из зашитого списка кодов PLAYBOOK р.8 (дерево регионов Wordstat живым инструментом не отдаётся); если кода нет / только страна - `213` + note. **Передавай числом.**
