...
| 6 | `A6.xlsx instruction row + headers at row 2 + data validation` | первая строка = инструкция клиенту, заголовки в строке 2, dropdown на колонке «Целевая?» |
| 7 | `import-structure.mjs all yes -> exit 0` | все «да» возвращают exit 0, stats.yes=4 |
| 8 | `import-structure.mjs mixed -> exit 3` | смесь «обсудить»/«нет»/«да» возвращает exit 3 |
| 9 | `import-structure.mjs all empty -> exit 4` | пустая колонка возвращает exit 4 |
...
└── structure_dir/
    ...
    ├── markers.json       # маркеры для 3 (info без маркера)
