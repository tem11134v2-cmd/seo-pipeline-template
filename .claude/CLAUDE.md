...
| `/seo-tekst [--from-structure NNN \| --from-table <путь> \| --from-analysis NNN \| --from-brief <путь>] [--mode A\|B] [--review \| --auto] [--theme <ниша>]` | Конверсионные тексты коммерческих страниц + HTML-прототип: анализ ЦА/оффера → согласование с клиентом (Analysis.docx → Google Doc, revising-цикл) → веер писателей → сборка прототипов поверх kit. Выход: Texts.docx (Google Doc) + prototype.html на страницу. **`--from-brief` - автономный режим без сеошки:** структуры и анализа нет, состав страниц собирается из брифа и подтверждается гейтом (для проектов, где SEO не покупается - например сайты партнёра-разработчика) | `texts/NNN-slug/` (per-task) |
