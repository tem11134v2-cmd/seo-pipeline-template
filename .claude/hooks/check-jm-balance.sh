...
#
# ВАЖНО: хук работает ТОЛЬКО при наличии .env с переменными JM_BALANCE_URL
# и JM_TOKEN (см. .env.example). Без них — молча пропускает проверку.
# Это сознательный no-op, чтобы пустая конфигурация не ломала пайплайн.
# Реальная страховка против пустого баланса — в промте jm-analyst,
# где явно вызывается jm_account перед text_analyze.
#
# Стратегия:
