...
```
/seo-statya <N | N-M | N,M,K | "тема текстом"> [--resume] [--review | --auto] [--with-handoff]
                   [--genre="<жанр>"] [--platform=site|external|social]
                   [--rebuild-docx] [--finalize-batch]
                   [--fast [--query="<основной запрос>"] [--words=N]]
```
...
- `--fast` — быстрый дешёвый режим: текст + метатеги + HTML, без фото/enhancer/docx/Drive/LLM-аудита и без полного JM-цикла (`text_analyze` не вызывается вообще). Один агент пишет статью целиком за один проход. `N` принимает и номер из `topics.xlsx`, и свободный текст темы прямо в команде (без обязательного `/seo-temi`). Опции: `--query="<основной запрос>"` (для свободной темы, иначе запрос берётся из самой темы), `--words=N` (целевой объём, по умолчанию ~1000). Полностью отдельный алгоритм — **FAST.md** (см. ниже, шаг 0b). Не смешивать с `--review`/`--rebuild-docx`/`--finalize-batch`.
...
**Быстрый режим — `--fast`.** Отдельный короткий пайплайн для больших объёмов (30-100+ статей), см. `FAST.md`. Определяется и обрабатывается на шаге 0b, до остальной логики этого файла.
...
**Если передан `--fast`** — сначала проверить несовместимые флаги: если
одновременно передан `--finalize-batch`, `--rebuild-docx` или `--review` -
**стоп** с сообщением «`--fast` нельзя сочетать с `--finalize-batch`/
`--rebuild-docx`/`--review` - это независимый упрощённый пайплайн, у него
нет пауз ревью и нет отдельной recovery-ветки docx». Не читать `FAST.md`.
(Без этого гарда `--fast --finalize-batch all` тихо трактует `"all"` как
свободную тему вместо режима финализации батча - т.к. `"all"` не проходит
числовой regex ниже.)

Иначе — определить источник темы ДО обычной детекции диапазона/списка
ниже: если позиционный аргумент `N` НЕ соответствует
`^[\d,\-\s]+$` (то есть содержит буквы) - это **свободная тема** (не номер
и не диапазон), даже если в тексте есть дефис или запятая (например «SEO-
продвижение сайта: с чего начать» иначе ложно распознается как диапазон).
Прочитать **`FAST.md`** целиком (та же папка, рядом с этим файлом) и
выполнить весь алгоритм по нему. К шагам 0c-15 этого файла (SKILL.md)
дальше не возвращаться — вся детекция диапазона/списка/`--finalize-batch`/
`--rebuild-docx` ниже относится только к обычному (не `--fast`) режиму.
