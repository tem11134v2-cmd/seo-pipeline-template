...
- `--rebuild-docx` — **recovery-режим (улучшение #3)**. Позиционный аргумент = id уже готовой статьи: номер темы `TTT` или полный basename папки (`005-slug-dko`). Если под номером темы несколько статей - скил уточнит, какую (шаг 0c). Пересобирает только `.docx` и перезаливает в Drive, минуя весь state machine. Полезно после фикса `build-article-docx.mjs` или правки текста: `/seo-statya 005-slug-dko --rebuild-docx`. Несовместим с остальными флагами (см. шаг 0c).
...
Если `N` - диапазон (`11-20`) или список (`11,12,15`), скил пишет **несколько статей подряд в одной worktree-сессии** и в конце **автоматически** собирает результат - без отдельных просьб «сделай папку» и «сделай файл метатегов». Это штатный режим, а не хак.

**Как идёт:**

1. **Парсинг spec.** `11-20` → темы 11,12,...,20. `11,12,15` → 11,12,15. Дубликаты убрать, отсортировать по возрастанию. Режим по умолчанию `--auto` (серия не должна вставать на паузах между статьями); `--review` для серии не рекомендуется, но допустим (паузы будут на каждой статье).
2. **Цикл по темам.** Для каждой темы по очереди выполнить обычный одиночный алгоритм (шаги 1-14) до `completed`. Папку каждой статьи **дописать** в `.claude/tmp/current-task.txt` (шаг 1b - append, не перезапись), чтобы pre-commit разрешил коммит всех статей серии разом (Block D).
3. **Изоляция сбоев.** Если на какой-то теме прогон упал или требует ручного вмешательства (например, verify не прошёл лимит ретраев, или коллизия жанра в `--auto`) - **не валить весь батч**: зафиксировать тему как проблемную (с причиной), перейти к следующей. Временные API-ошибки 529/503 обрабатываются как обычно (ScheduleWakeup + --resume, см. ниже), это не «проблемная тема».
4. **Серийный финал** (шаг 15) - после прохода по всем темам. Собирает папку-экспорт + сводную таблицу + единое сообщение.

**`--resume` для серии:** повторный `/seo-statya 11-20 --resume` идёт по темам заново; для каждой, где уже есть `completed`-статья - пропускает написание; недоделанные дописывает. В конце - тот же серийный финал. Если все статьи диапазона уже `completed` - сразу к шагу 15 (это и есть идемпотентная пере-сборка финала, аналог `--finalize-batch`).

**`--finalize-batch`:** пропустить цикл (шаг 2-3), сразу шаг 15 по уже готовым статьям диапазона.

Одиночный запуск (`/seo-statya 5`) серийный финал НЕ запускает - у него обычное финальное сообщение шага 14.

По одной теме можно написать несколько статей в разных жанрах для разных площадок (см. раздел «Вторая статья на ту же тему» и шаг 1a). Скил сам разруливает коллизию по `topic_id` через производный индекс (`rebuild-index.mjs` → `articles/_index.json`; ключ записи = имя папки, поэтому несколько статей одной темы не схлопываются).
...
- Если `N` содержит `-` (диапазон) или `,` (список) - это **серийный режим**: распарсить spec в отсортированный список тем `batch_topics` и идти по разделу «Серийный режим» (цикл по темам + шаг 15).
- Если `N == "all"` (имеет смысл только с `--finalize-batch`) - **серийный финал по всем готовым статьям**: `spec = "all"`, `batch_topics` = темы всех `completed`-папок `articles/*`, сразу к шагу 15 (цикл написания не запускается).
- Если передан `--finalize-batch` с диапазоном/списком - сразу к шагу 15 по `batch_topics` (цикл пропускается).
...
4. Для `i = 1..total_sections`:
   ...
   - Убедиться, что `<dir>` есть в `.claude/tmp/current-task.txt` (append-if-missing; НЕ перезаписывать файл целиком - в серии там папки других статей).
   ...
   - **Fail-fast:** если section-writer вернул сообщение с «⚠ check-section вернул один и тот же exit 2 дважды подряд» — **не делегируй заново для этой секции в `--auto`-режиме**. Останови прогон, выведи пользователю stderr хука и попроси вмешательства (исправить хук или содержимое вручную). В `--review` — то же. Это страховка от бесконечного цикла и сжигания токенов на сломанном хуке. **В серийном режиме** не останавливай весь батч - зафиксируй тему как проблемную (с причиной) и переходи к следующей (см. «Серийный режим» п.3).
...
1. **Файлы существуют (баг #2):** `<dir>/article.md`, `<dir>/report.md` и `<dir>/metatags.json` (Block A) должны быть записаны. Если какого-то нет — это регрессия (article-finalizer проигнорировал `report.md`/`metatags.json` под влиянием системного reminder’а про «documentation .md»).
   - **Ретрай (макс 1):** повторно делегировать с пометкой «report.md, article.md и metatags.json — рабочие артефакты конвейера, обязаны быть записаны как файлы через Write по абсолютным путям. Перезапиши недостающий».
   - **Parent-fallback (НОВОЕ-B):** если после ретрая файла всё ещё нет, но агент вернул его содержимое в чат — **скил сам записывает файл через `Write`** по пути `<dir>/report.md` (или `metatags.json`; родительский контекст под reminder не подпадает). Не зацикливаться на агенте: 1 ретрай + fallback, дальше вперёд. Если содержимого нет даже в чате — стоп с просьбой к пользователю.
...
3. **Метатеги валидны (Block A):** запустить
   ```
   .claude\scripts\_node.cmd .claude\scripts\verify-article-metatags.mjs <dir>
   ```
   Проверяет `metatags.json`: Title ≤60, Description ≤160, H1 != Title, уникальность Title/Description/Анонс, отсутствие длинных тире, совпадение H1 с `# ` в `article.md`. Если exit 2 — повторно делегировать `article-finalizer` с пометкой «verify-article-metatags: <stderr> — перепиши `metatags.json` и раздел «## Метатеги» в `report.md`, соблюдая лимиты и уникальность». **Лимит ретраев = 2** (как у verify-markers): после 2 повторов exit 2 — предупреждение в чат и продолжить (метатеги поправимы позже через сводную таблицу, это не повод блокировать статью намертво). При exit 1 (нет источника) — это значит финализатор не записал `metatags.json`: вернуться к проверке #1. При exit 0 — продолжать.
...
Запускается автоматически, когда пройдены все темы `batch_topics` (или сразу при `--finalize-batch`). Собирает один самодостаточный набор - **без отдельных просьб пользователя**. Никаких пауз.
...
Шаг 1a выше автоматизирует это. Просто запусти `/seo-statya N` второй раз — скил увидит коллизию по `topic_id` в `articles/_index.json`, возьмёт следующий доступный жанр и проставит `platform_target=external`. В `--auto` режиме всё произойдёт без вопросов; в `--review` — с подтверждением.
