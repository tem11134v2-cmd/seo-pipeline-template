...
// args: { root, slug, findings?: [пути], round?: 3, skipJudge?: boolean, model?, model_light? }
...
const fixer = (list, label, phase) => agent(`${pre('prompts/06-fixer.md')}\nПараметры: slug=${slug}; findings=${list.join(',')}.`, { label, phase, effort: 'high', model: MODEL, schema: FIX })
...
if (!(args && args.skipJudge)) {
  phase('Judge')
  judge = await agent(`${pre('prompts/06-page-judge.md')}\nПараметры: slug=${slug}; round=${round}; scope=full. Это финальный круг после лечения повторов: проверь всю страницу заново по всем пунктам, особенно повторы по счету (п. 9), название компании (п. 10) и связность переходов после правок.`, { label: `judge:${round}`, phase: 'Judge', effort: 'high', model: MODEL, schema: FINDINGS })
  if (serious(judge).length) fix2 = await fixer([`work/audit/${slug}/round-${round}.json`, lintFile], 'fix:2', 'Judge')
}

phase('Build')
const build = await agent(`Запусти команду из папки проекта ${ROOT}:\ncd "${ROOT}" && node scripts/lint-page.mjs ${slug}; node scripts/render-md.mjs ${slug} && node scripts/build-html.mjs && node scripts/check-html.mjs; node scripts/report.mjs\nНичего не исправляй. Верни ok (код выхода 0), exit_code и последние 40 строк вывода.`, { label: 'build', phase: 'Build', effort: 'low', model: MODEL_LIGHT, schema: RUN })
