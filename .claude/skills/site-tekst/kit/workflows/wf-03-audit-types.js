...
// args: { root, types:[...], type_pages?:{type:N}, small_type_max?: 2, small_batch?: 4, solo_types?: ["home"], model?, model_light? }
...
if (!typePages) {
  const prep = await agent(`Запусти команду из папки проекта ${ROOT}:\ncd "${ROOT}" && node scripts/prep-args.mjs --types ${types.join(',')}\nНичего не исправляй. stdout команды - JSON: перенеси type_pages как список {type, pages}. ok - код выхода 0; при ошибке - текст в error.`, { label: 'prep-args', phase: 'Audit', effort: 'low', model: MODEL_LIGHT, schema: PREP })
...
const results = await pipeline(groups,
  g => agent(`${pre('prompts/03-type-auditor.md')}\nПараметры: types=${JSON.stringify(g)}; round=1.`, { label: `audit:${key(g)}:1`, phase: 'Audit', effort: 'high', model: MODEL, schema: AUDIT }),
  async (r1, g) => {
    const one = byType(r1)
    ...
    const fix = byType(await agent(`${pre('prompts/03-type-fixer.md')}\nПараметры: types=${JSON.stringify(bad)}; round=1.`, { label: `fix:${key(bad)}`, phase: 'Audit', effort: 'high', model: MODEL, schema: FIX }))
    const two = byType(await agent(`${pre('prompts/03-type-auditor.md')}\nПараметры: types=${JSON.stringify(bad)}; round=2.`, { label: `audit:${key(bad)}:2`, phase: 'Audit', effort: 'high', model: MODEL, schema: AUDIT }))
