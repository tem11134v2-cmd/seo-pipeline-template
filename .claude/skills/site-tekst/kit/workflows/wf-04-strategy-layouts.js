...
//   small_type_max?: 2, small_batch?: 4, solo_types?: ["home"], model?, model_light? }
...
if (!typePages) {
  const prep = await agent(`Запусти команду из папки проекта ${ROOT}:\ncd "${ROOT}" && node scripts/prep-args.mjs --types ${types.join(',')}\nНичего не исправляй. stdout команды - JSON: перенеси type_pages как список {type, pages}. ok - код выхода 0; при ошибке - текст в error.`, { label: 'prep-args', phase: 'Strategy', effort: 'low', model: MODEL_LIGHT, schema: PREP })
...
const strategyChain = async () => {
  ...
  const briefs = await agent(`Выполни из папки проекта ${ROOT} две команды по очереди, вторую - даже если первая завершилась с ошибкой:\n1) cd "${ROOT}" && node scripts/merge-strategy.mjs\n2) cd "${ROOT}" && node scripts/build-briefs.mjs\nНичего не исправляй и не интерпретируй. Верни ok (обе с кодом 0), exit_code (первый ненулевой код или 0) и последние 60 строк вывода обеих команд, каждую с заголовком «merge:» / «briefs:».`, { label: 'merge+build-briefs', phase: 'Briefs', effort: 'low', model: MODEL_LIGHT, schema: RUN })
...
const layoutChain = () => pipeline(groups,
  g => agent(`${pre('prompts/04-layout-generator.md')}\nПараметры: types=${JSON.stringify(g)}; round=1.`, { label: `layout:${key(g)}`, phase: 'Layouts', effort: 'medium', model: MODEL, schema: LAYOUT }),
  async (r, g) => {
    const one = byType(r)
    const redo = g.filter(t => !one[t] || one[t].validator !== 'pass')
    if (!redo.length) return g.map(t => one[t])
    const two = byType(await agent(`${pre('prompts/04-layout-generator.md')}\nПараметры: types=${JSON.stringify(redo)}; round=2. По каждому типу прочитай work/audit/layouts/<type>.json и исправь.`, { label: `layout:${key(redo)}:2`, phase: 'Layouts', effort: 'medium', model: MODEL, schema: LAYOUT }))
