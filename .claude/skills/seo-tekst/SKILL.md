...
### 6b. Копи-валидатор - шлюз перед HTML (state == texts-written)
Отдельный проход качества (можем позволить - в отличие от claude.ai). Делегировать `copy-auditor` **на каждую страницу веером** (6-8): читает page.json + COPY.md (13-пунктовый чек-лист + стоп-листы v4) + facts.json, **чинит** нарушения на месте (самозащита/жаргон/аббревиатуры/манипуляции/арифметика/повторы/H1/«за счёт»/OPSEC). Затем по каждой странице:
```
