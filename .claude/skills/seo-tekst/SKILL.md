...
**Лексикон, половина 1** (`translate` + `locked` по основаниям а/б) - собрать сейчас, см. `assets/ORCHESTRATOR-REF.md`, раздел «Словарь проекта». `update-meta.sh <texts_dir> strategy-done`.
...
Превью - заказчику ФАЙЛОМ (`tone/tone-preview.html`) И в Drive КАК ФАЙЛ (`assets/ORCHESTRATOR-REF.md`, блок «Drive»: `convertToGoogleFormat:false`, постоянная ссылка) -> `share.json.tone_preview`; Drive недоступен / нет `texts_folder_id` - не блокировать: локальный файл + подсказка `/share-tekst <NNN>`.
...
- **Выбор варианта** -> `tone-chosen`:
  ...
  4. Перенести подтвержденный канон в `facts.json.lexicon.canonical` (`assets/ORCHESTRATOR-REF.md`, раздел «Словарь проекта»); правки записки применить к `strategy.canonical_wordings` до переноса.
...
- `prototype.html` - заказчику ФАЙЛОМ И в Drive КАК ФАЙЛ (`assets/ORCHESTRATOR-REF.md`, блок «Drive»: `convertToGoogleFormat:false`, постоянная ссылка) -> `share.json.prototype` (запись скриптом `share-record.mjs`, схема `{drive_file_id, drive_link, mime_type, shared_at, revisions[]}`); Drive недоступен / нет `texts_folder_id` - не блокировать: локальный файл + подсказка `/share-tekst <NNN>`;
...
## Справочник оркестратора (вынесен)

Три справочных блока живут в `assets/ORCHESTRATOR-REF.md` и читаются ТОЧЕЧНО, по ссылке из шага:

- **Словарь проекта (`facts.json.lexicon`)** - половина 1 на шаге 3, половина 2 на шаге 6;
- **Drive** - три загрузки задачи (скелеты 4b, тон-превью 5e, прототип 7e) + запись в `share.json` скриптом;
- **Если pre-commit отказал** - таблица «ситуация -> действие».
