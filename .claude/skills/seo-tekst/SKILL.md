...
**Два уровня работы:**
- **Проектный (один раз):** анализ ЦА (`audience-analyst`) + стратегия оффера (`offer-strategist`) + после гейта - блок-план всех страниц (`block-planner`). Результат анализа согласуется с заказчиком (Google Doc) ДО написания текстов.
- **Постраничный (веером, параллельно):** `page-writer` пишет копию по своему blueprint (каталоги BLOCKS/COPY он НЕ читает - только VOICE.md, контекст писателя тощий, см. ADR-020) -> `copy-auditor` чистит -> `prototype-builder` собирает HTML поверх kit. Десяток страниц идут пачками.
...
Маркер: `.claude/tmp/expected-block-planner-<run_id>.txt = <texts_dir>/blueprints/` (проверка - наличие папки с файлом на каждую страницу). Делегировать `block-planner` (один вызов на весь проект): `texts_dir`, `project_root`, `mode`. Он читает BLOCKS.md + leader_blocks.json + fragments-manifest + strategy и пишет `blueprints/<page-slug>.json` на каждую страницу (блоки + цели + боли + слоты + char-лимиты). **Писатели каталоги не читают - только свой blueprint** (диета контекста, ADR-020). После: проверить, что blueprint есть для каждой страницы из pages.json; недостающие - пере-делегировать (до 2). State не меняем.

### 6b. Тексты - веер писателей (state == approved, blueprints готовы)
...
### 6c. Копи-валидатор - шлюз перед HTML (state == texts-written)
Отдельный проход качества (можем позволить - в отличие от claude.ai). Делегировать `copy-auditor` **на каждую страницу веером** (6-8): читает page.json + COPY-AUDIT.md (14-пунктовый чек-лист: стоп-листы v4 + п.14 маркеры ИИ-текста) + facts.json, **чинит** нарушения на месте (самозащита/жаргон/аббревиатуры/манипуляции/арифметика/повторы/H1/«за счёт»/OPSEC/ИИ-маркеры). Затем по каждой странице:
```
.claude\scripts\_node.cmd .claude\scripts\verify-copy.mjs texts/NNN/pages/<slug>/
```
Exit 2 (механические нарушения остались) - пере-делегировать `copy-auditor` этой страницы со списком (до 2). Аналог post-валидатора HTML, но для копирайта: не прошёл - правим ТЕКСТ, прототип не собираем. State остаётся `texts-written`.
