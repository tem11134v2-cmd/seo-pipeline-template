...
├── site_audit.json        # кросс-страничный аудит: дубли/ИИ-узор сайта/уникальность H1 + вердикт (site-reviewer)
...
То, что не видно на одной странице: сайт из 10 хороших, но написанных под копирку страниц - это ИИ-сайт. Маркер: `.claude/tmp/expected-site-reviewer-<run_id>.txt = <texts_dir>/site_audit.json`. Делегировать `site-reviewer` (один вызов на проект): `texts_dir`. Он читает ВСЕ pages/*/page.json + VOICE.md + decisions и: межстраничные дубли формулировок -> перефразирует; ИИ-узор сайта (одинаковые зачины/плашки/CTA везде) -> расшатывает; H1/Title - уникальность; consistency decisions; конфликты цифр между страницами -> по facts.json. После его правок - повторно `verify-copy.mjs` по затронутым страницам (он мог менять текст; exit 2 -> пере-делегировать copy-auditor этой страницы, до 1 раза). `update-meta.sh <texts_dir> site-reviewed`.
...
## Параллельная работа
Несколько прогонов - каждый в своём worktree, состояния не пересекаются. Внутри прогона вееры идут пачками: direction-scanner 4-6, page-writer / copy-auditor / prototype-builder 6-8 (cap против overload). Одиночные агенты (audience/offer/block-planner при <=12 стр./site-reviewer) - с expected-маркером, вееры - без.
