...
Плитку тегов, перелинковку и SEO-нормализацию НЕ делает - это `/seo-faq`. **Вопросы и ответы - две разные вещи, и граница проходит по работе, а не по формату:** конверсионный блок «Вопросы и ответы» (5-8 вопросов из `audience.json`, снятие возражений, без Schema.org и без подбора запросов) здесь РАЗРЕШЕН при любом tier - это блок 33к каталога; SEO-FAQ (разметка FAQPage + добор недостающих N-грамм по JM) остается платной работой `/seo-faq`. Мотив: при `tier=basic` SEO не покупают, и прежний запрет «FAQ - зона /seo-faq» означал, что блок не появится НИКОГДА, даже когда заказчик просит его прямым текстом (боевой прогон: попросил дважды, планировщик подставил вместо него чужой блок с техническим именем).
...
├── task-context.md                # оркестратор: расшифровки имен, решения гейтов дословно, ограничения - то, что иначе копипастится в каждый промт веера
...
- **Пустое поле и решение заказчика - разные вещи.** Телефона у заказчика нет вовсе -> `legal.phone_absent: true` (блок телефона не печатается нигде, `verify-prototype` перестает требовать `tel:`). Офлайна и графика нет -> `schedule` не заполнять: ассемблер печатает график ТОЛЬКО из данных, дефолтной строки в коде нет. Обратное - выдуманный факт в шапке, который не ловит ни один верификатор (они читают `page.json`, а не оболочку).
- **Реквизиты, которые мост не вычленил**, лежат в `facts.json.jur.requisites_unresolved[]` сырыми строками с причиной (кандидатов несколько / в строке проза). Поля при этом пустые - это штатно: скрипт не угадывает, закрываешь ты на ревизии ниже.
- **`inputs.json.region_yandex`** - если мост оставил null (путь --from-analysis: brief несет имя региона, не код) - дописать код Яндекса ЧИСЛОМ по `region_name` (справочник - `.claude/skills/seo-metategi/PLAYBOOK.md` р.8); без кода jm_wordstat стратега тихо выродится в федеральную частотность. **Ветка «проект без гео»** (полностью онлайн, доставка по стране, услуги без привязки к городу - видно по `region_name`/`region_note`): код НЕ дописывать, оставить `null` и пометить строкой в сводке. Мотив правила тут не применим: для такого проекта федеральная частотность и есть верная, а подставленный код города сузит выдачу до чужого рынка. `region_name` - короткое имя региона; полная формулировка брифа, если она была абзацем, лежит рядом в `region_note`.
...
Мост размечает семена машинно, ревизия их ПОДТВЕРЖДАЕТ, а не создает с нуля: `label_auto: true` - привязка выведена из значения (проверь, что она верна); `publish: "no"` + `publish_why` - число снято по маркеру («снято», «не публиковать») или совпало с `forbidden_wordings` брифа (публикуемое верни в `"as-is"`); `jur.requisites_unresolved[]` - реквизит не вычленен однозначно, поле пустое. Классификатор намеренно строг в одну сторону: лишний вопрос на ревизии дешевле, чем снятая заказчиком цифра, уехавшая на живой сайт.
...
**`task-context.md` - контекст задачи одним файлом (пишешь ты, один раз; дополняешь после каждого гейта).** Кто читает: писатели, копи-аудитор, site-reviewer - им путь передается строкой в промте. Зачем: то, чего нет ни в одном артефакте, приходилось вписывать в промт КАЖДОМУ агенту веера отдельно, слово в слово (на боевом прогоне - трижды подряд одному и тому же тексту), и любая правка этого текста жила ровно один вызов. Файл держи коротким (до ~60 строк), только то, что нельзя вычитать из `strategy.json`, `facts.json` и blueprint:
- **расшифровка имен**: какой блок что означает на этом проекте (каталожное имя -> о чем блок), если имена повторяются;
- **решения гейтов дословно** (состав, скелеты, тон) - что заказчик снял, что попросил добавить, какими словами;
- **ограничения**: opsec, запрещенные формулировки, чего нет в ассортименте - одной строкой каждое;
- **что решено про спорные места** (кого не называем, что не обещаем, какие цифры сняты и почему).
Все, что доехало до `strategy.json`/`facts.json`/`blueprint`, сюда НЕ дублируй: два источника одного факта разъедутся на первой же правке.
...
3. **Правки** -> `update-meta.sh <texts_dir> skeletons-revising`. Сначала **снять комментарии заказчика** с текущего документа (`listComments` по `share.json.skeletons.drive_file_id` -> `skeletons_comments.json`, блок «Drive»): перезаливка создаст НОВЫЙ документ, и на прежнем они останутся - сверять «что из десяти учтено» иначе придется руками по двум вкладкам. Затем `block-planner` такт 1 ТОЧЕЧНО: в промте перечислить только изменившиеся типы и правки заказчика дословно, скелеты остальных типов не трогать. Затем re-docx (п.1) + re-upload с `--revision skeletons_gate` -> обратно `skeletons-shared`. В сообщении заказчику - свежая ссылка + строка «комментарии к прошлой версии учтены» со списком закрытого. Цикл до согласования.
...
Контекст задачи: task-context.md - прочитай ПЕРВЫМ (расшифровки имен, решения гейтов, ограничения).
...
```
Передать `copy-auditor` (веер 6-8, без маркеров) **весь вывод - и нарушения, и предупреждения** (`!`/`~` по коду возврата не видны, а решение по ним принимает редактор) плюс строку «Контекст задачи: task-context.md - прочитай ПЕРВЫМ». Аудитор чинит на месте по приоритету: смысл и грамотность -> удар в боль ЦА -> чистота (утечка кухни, жаргон, штампы, манипуляции, повторы, лимиты). Анти-ИИ не делает (ADR-022).
...
```json
{ "pages": [ { "slug": "<slug>", "title": "<название для списка и для меню>", "type": "<тип>", "order": <n>,
              "url": "<адрес на живом сайте, опц.>", "parent": "<slug родителя, опц.>", "nav": false } ],
  "start": "__index", "main_slug": "<main_slug>" }
```
Три опциональных поля - от них зависит навигация собранного документа, и заполняет их только оркестратор (в `pages.json` их нет):
- **`url`** - адрес страницы на живом сайте (из `pages.json.url`). По нему ассемблер переводит относительные ссылки писателей (`/arhitektura/`) в маршруты роутера `#p/<slug>`. Не проставишь - ссылка останется относительной и уведет заказчика из документа; `verify-prototype` это блокирует.
- **`parent`** - slug родительской страницы: из этого строится меню второго уровня. Не проставлен - фолбэк по `dir_slug` (страница с таким слагом существует), иначе пункт верхнего уровня. Меню плоское - не авария.
- **`nav: false`** - убрать страницу из меню (служебные, юридические). По умолчанию в меню все.
