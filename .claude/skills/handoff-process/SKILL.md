...
### 8. Напомнить о незакрытых worktree (оппортунистически)

handoff-process всё равно в main - данные под рукой, дёшево заглянуть. После обработки:

```bash
git worktree list
```

Если есть живые worktree кроме main (пути `.claude/worktrees/<name>` на ветках `claude/*`) - одна строка-напоминание:
> В проекте ещё N живых worktree: `<имена>`. Если там незавершённая работа - продолжи в той сессии; если работа готова, но `/handoff` забыли - открой worktree и сделай `/handoff`. Полный срез (что готово / застряло / zombie по всем проектам) - `/status` из `~/seo-projects/`.

**НЕ** предлагать авто-handoff или удаление живого worktree (особенно с несмерженными коммитами - потеря работы). Это только напоминание; разбор и осторожные действия - в `/status`. Зомби-папки (без живого worktree) уже подчищены шагом 1.
