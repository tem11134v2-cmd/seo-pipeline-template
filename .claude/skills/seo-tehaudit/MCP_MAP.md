...
### Шаг 3 - audit-onpage (URL/мета/Schema)

| Тул | Что даёт | Лимит |
|---|---|---|
| `domain_pages` | страницы домена с числом запросов в ТОП (для выборки), `base="<recon.keyso_base>"` | 1 |
| `mcp_fetch_page` | HTML 8-12 страниц выборки (Title/H1/Desc/noindex/canonical/Schema/favicon/JS) | 8-14 |
...
- Шаг 3 (`audit-onpage`) и шаг 4 (`audit-analytics`) **независимы** - оба читают только `recon.json` + `indexing.json`. Запускать их **в один параллельный заход** (две делегации в одном сообщении). Ни один не использует Арсенкин.
...
**Бюджет:** ~35-55 MCP-вызовов на аудит (recon ~8, indexing ~15-25, onpage ~10-15, analytics ~6). Если упёрся в лимит - не дробить дальше, идти к следующему шагу с тем что есть.
