...
- Прочитать `meta.json`. `state = meta.state`, `mode = meta.mode || "auto"`.
- В `--auto` режиме — перейти сразу к следующему шагу после `state`. В `--review` — спросить пользователя «Найдено в состоянии `<state>`. Продолжить? [Y/n]».

Иначе:
- **Проверка на повторный запуск той же темы:** если в `articles/` уже есть папка `<NNN>-<slug>` (тот же slug), значит статья по этой теме уже написана. Спросить пользователя (в обоих режимах — это решение про скоуп, не про темп):
  ...
  Если пользователь согласен — продолжить с NNN+1 (новая папка `articles/<NNN+1>-<slug>/`), жанр взять из ответа. Если отказался — стоп.
...
```
- Записать `.claude/tmp/current-task.txt` с путём к `dir/` (используется хуками `check-section.sh`, `mark-finalized.sh`, и **критично — pre-commit hook'ом**, который без этого файла откажет в коммите).
- `state = "init"`.
...
Типичный сценарий: одна и та же тема — две статьи разных жанров:
...
**JM-кеш экономит лимиты:** `jm_text_analyze` кешируется на 4 часа по тем же параметрам, поэтому второй прогон в тот же день не тратит ≥5 лимитов на JM-анализ повторно.

**Жанр на второй прогон:** скил спросит «уже есть статья по теме N — какой жанр взять?». Возьми второй из колонки «Жанры» в `topics.xlsx`.
