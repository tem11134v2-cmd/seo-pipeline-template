...
```
Читает `inputs.json` + `pages.json` + `pages/<n>.json`, пишет `A7_<slug>.xlsx` (3 листа). `update-meta.sh <metatags_dir> xlsx-built`. Переход к шагу 7.5.

### 7.5. Финальная проверка (state == "xlsx-built", перед Drive)

Легкий финальный гейт ПОСЛЕ сборки xlsx, ПЕРЕД заливкой в Drive. Нового state НЕ вводим - остаемся в `xlsx-built` (чтобы не плодить рябь на resume). Две части: механический прогон + смысловой спот-чек силами оркестратора (без нового агента).

#### 7.5а. Механический финальный прогон

```
.claude\scripts\_node.cmd .claude\scripts\verify-metatags.mjs <metatags_dir> --accept-degraded
```
Финальная страховка после всех ре-делегаций шага 6: ловит расхождения, если правка одной страницы затронула другую. `--accept-degraded` - деградацию уже приняли на шаге 6, здесь она не должна повторно блокировать (missing/violations по-прежнему дают exit 2).
- **Exit 0** - к спот-чеку 7.5б.
- **Exit 2** - регрессия (появились missing/violations после шага 6). Пере-делегировать затронутые страницы (как шаг 5, по одной в deep), пересобрать xlsx (шаг 7), повторить 7.5а. **Максимум 1 круг** (страховочный; штатно после шага 6 быть не должно). Если не сходится - отметить в финальной сводке и продолжить (xlsx уже собран).
- **Exit 1** - ошибка запуска (нет pages.json) - показать stderr, стоп.

#### 7.5б. Смысловой спот-чек (сам оркестратор, без нового агента)

Оркестратор выбирает 3-5 страниц (приоритетные/высокочастотные или репрезентативная выборка по типам) и читает их `pages/<n>.json`. По каждой - легкий чек-лист:
- H1/Title осмысленны, маркер вписан естественно (не «маркер | маркер | маркер»).
- УТП в Description выглядят взятыми из `inputs.json` (`utp_*`/`assortment`), а не выдуманными.
- Description дополняет Title, не дублирует; есть внятный CTA.
- Нет банальностей/кликбейта/канцелярита (PLAYBOOK р.1-3 запрещенные).

Если спот-чек вскрыл системную проблему (напр. на 3 из 5 страниц выдуманные УТП) - пере-делегировать затронутые страницы с явной инструкцией (как шаг 5), пересобрать xlsx (шаг 7), повторить 7.5. Разовая косметика - отметить в сводке, не пере-делегировать. Это НЕ новый агент и НЕ полный обход всех страниц - дешевая выборочная вычитка глазами оркестратора (opus), дополняющая механику `verify-metatags.mjs`.

**Границы шага 7.5:** `PLAYBOOK.md` (лимиты 60/160) НЕ трогаем, deep/bulk-механику НЕ трогаем - спот-чек только читает готовые `pages/<n>.json`. Нового state нет: после 7.5 остаемся в `xlsx-built` и идем на шаг 8.
...
- НЕ запускай `/seo-struktura`, `/seo-statya`, `/seo-strategiya`, `/site-analiz` из этой сессии - отдельные worktree-задачи.
