...
### 1.5. Проверка незавершенного анализа (гейт `sites/`)

Анализ `/site-analiz` списка состояний не ведет: что сделано, видно по файлам, а согласование с заказчиком - одно поле `queue.json` -> `gate.approved`, его ставит `queue.mjs gate --by`. Структура и тексты читают анализ только после гейта, поэтому анализ, сданный без него, - это задача, которую дальше никто не возьмет.

Прочитать `.claude/tmp/current-task.txt`. Для каждой строки вида `sites/NNN-*/` прочитать `<папка>/queue.json` и вывести шаг:

```bash
node .claude/scripts/site/queue.mjs state <папка>
```

Если `gate.approved` не `true` (гейта нет, файла `queue.json` нет или он не разбирается):

> ⚠️ Анализ `sites/<NNN>-<slug>/` не прошел гейт заказчика (следующий шаг: `<шаг из queue.mjs state>`).
> Структура (`/seo-struktura`) и тексты (`/site-tekst`) его не примут; тексты - только явным `--allow-ungated`.
>
> Точно делаешь /handoff? Варианты:
>   [Y] - да, сдать как есть (документы у заказчика, ответы придут позже - гейт закроется следующей сессией)
>   [n] - нет, сначала довести до гейта (вернуться в скил: ответы -> `apply-answers.mjs` -> `queue.mjs gate`)
>   [skip] - да, и пропустить эту проверку для остальных задач в worktree

Если N - стоп. Если Y или skip - продолжить как обычно.

Если `gate.approved === true` - без предупреждения, продолжать. Задачи других типов (статьи, структура, тексты и т.д.) эту проверку не проходят. Старые папки `analyses/` (анализ v7 выведен) не проверяются: новых задач там нет.
