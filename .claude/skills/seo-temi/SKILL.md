...
**Источник 1 - корневой `topics.xlsx` (компактно, флаг `--main-only`):**
```
.claude\scripts\_node.cmd .claude\scripts\read-topics-xlsx.mjs . --main-only
```
Печатает только `{ exists, count, main_queries }` - без полного дампа тем (полный JSON темника в контекст не тянуть). Если `exists: true` - забрать `main_queries` (уже без пустых).
...
**Записать результат в файл** `topics/NNN-<slug>/existing_queries.json`:
```json
{ "main_queries": ["...", "..."], "count": N, "sources": ["topics.xlsx", "articles/_index.json"] }
```
Это стоп-лист для `topics-verifier` (проверяет дубли по файлу, не по простыне в промте). `sources` - только те источники, которые реально дали данные (если `topics.xlsx` не существует - его в список не включать).
...
### 4.5. Проверка батча (перед показом пользователю)

Маркер:
```
.claude/tmp/expected-topics-verifier-<run_id>.txt:
  <abs path>/topics/NNN-<slug>/verify_report.json
```

Делегировать `topics-verifier`:
```
topics_dir: topics/NNN-<slug>/
project_root: <abs>

Прочитай topics-batch.json + existing_queries.json + ЗАКАЗЧИК.md. Проверь: нет дублей с
существующим темником и внутри батча, темы коммерчески релевантны нише, поля полные.
Ничего не чини. Запиши verify_report.json.
```

Прочитать `verify_report.json` точечно (`verdict` + `counters`, не весь файл):
- `verdict == pass` - к шагу 5 (показать таблицу пользователю, пауза на подтверждение).
- `verdict == needs-fix` / `fail` - пере-делегировать `topic-generator` с issues из отчета
  (например «эти main_query - дубли, замени», «тема N нерелевантна нише»), максимум 2 повтора
  суммарно, затем снова шаг 4.5.
  После 2 повторов без `pass` - НЕ блокировать: показать таблицу пользователю с пометкой
  оставшихся замечаний (финальное слово за пользователем - дубли и слабую релевантность он
  снимет правкой на шаге 5).

Примечание: генератор уже вывел полную таблицу на шаге 4. Верификатор работает по файлу, в чат -
только сводка (<= 5 строк). Полную таблицу повторно не печатать.
...
После применения правок вывести ТОЛЬКО дифф - строки, которые поменялись (не полную таблицу):
```
| Действие  | № | main_query | Что изменилось |
| убрана    | 7 | ...        | -              |
| добавлена | 26| ...        | новая тема про X |
| правка    | 5 | ...        | жанр -> Личный опыт |
```
Полную таблицу всех тем выводить ТОЛЬКО по явной просьбе пользователя («покажи все темы», «покажи таблицу целиком»). Первый вывод полной таблицы (в шаге 4, от topic-generator) это не отменяет - там она нужна для первичного согласования.
