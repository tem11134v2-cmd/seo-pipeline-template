...
| Тул | Что даёт | Шаг | Лимит |
|---|---|---|---|
| `arsenkin_commerce` | Пакетная проверка коммерциализации запросов (`commercial_pct` 0-100 на каждый) | 2 (**обязательно** после определения всех маркеров - страховка от «коммерческая страница под info-запрос») | 1-3 |
| `arsenkin_top` | ТОП-10 SERP по запросу с сниппетами | 2 (опц. при разборе пограничных случаев после `arsenkin_commerce`) | 0-3 |
...
9.  arsenkin_commerce(пакетно, все маркеры)      # ОБЯЗАТЕЛЬНО - проверка коммерциализации
10. keyword_similar(× 0-5)                        # для info-dominant маркеров - поиск синонимов
11. arsenkin_commerce(пакетно, кандидаты)        # повторная проверка синонимов
12. arsenkin_top(× 0-3)                           # опц. при разборе borderline
...
**Бюджет:** ~35-65 MCP-вызовов на структуру (с учётом 1-9 на arsenkin_commerce). JM `semantic_pack` - один большой вызов (это норма для JM).
...
| Превышен бюджет (>65 вызовов) | Прекратить добор фолбэков, перейти к следующему этапу с тем что есть |
