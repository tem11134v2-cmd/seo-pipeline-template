...
КЛИЕНТ ~/seo-projects/<slug>/   (НЕЗАВИСИМЫЕ направления, в любом порядке/параллельно)
  ...
  │    [планируется: /seo-tekst (продающие), /seo-faq]                │
  │                                                                   │
...
## 2. Три независимых направления (НЕ путать в одну цепочку)
...
`/seo-analiz` -> `/seo-struktura` -> [планируется: `/seo-tekst` (продающие тексты), `/seo-faq` (SEO-тексты в FAQ)]. Работает от брифа клиента, **НЕ требует** `/seo-shablon`. Единственная связь - `/seo-struktura` читает выход `/seo-analiz` (brief/competitors/serp/leader_scan), и она ВНУТРИ трека.
...
- **Дальше читает:** `/seo-struktura` берёт `brief.json` + `competitors.json` + `serp.json` + `leader_scan.json` (внутри коммерческого трека). `recommendations.json` - результат этого же трека, вход для будущих `/seo-tekst`/`/seo-faq`.
...
/seo-struktura --> A6.xlsx (клиенту) --> client_filled.xlsx --> structure_data.json --> A6.md
        |
        +--> A6.md -> вход для будущих /seo-tekst, /seo-faq (и ТЗ верстальщика)
