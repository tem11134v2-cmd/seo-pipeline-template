...
- `/guide <тема>` - показать только нужный раздел. Распознавай тему по ключу:
  - имя команды без слеша: `seo-shablon`, `site-analiz`, `seo-struktura`, `site-tekst`, `seo-metategi`, `seo-tehaudit`, `seo-strategiya`, `custom-question`, `seo-temi`, `seo-statya`, `seo-faq`, `fix-article`, `rewrite-section`, `handoff`, `handoff-process`, `request-shared-edit`, `share-*`, `new-project`, `status`, `sync-from-template`, `sync-all` -> раздел 3 по этой команде.
...
КЛИЕНТ ~/seo-projects/<slug>/   (НЕЗАВИСИМЫЕ направления, в любом порядке/параллельно)
  ...
  │                                                                   │
  │  ТРЕК «Сайт» (анализ -> [структура при SEO] -> тексты; от брифа,  │
  │  /site-analiz - для ВСЕХ клиентов, tier seo/basic):               │
  │    /site-analiz       -> sites/NNN: project.json + 2 документа    │
  │                          заказчику + гейт; basic - состав страниц │
  │    /seo-struktura NNN -> A6.xlsx -> клиент -> A6.md (нужен tier=seo)│
  │    /seo-metategi      -> A7.xlsx (метатеги H1/Title/Description)  │
  │    /site-tekst        -> texts/KKK: тексты + prototype.html       │
  │                          (гейты карты, стратегии, пилота, волны 1)│
  │    /seo-faq           -> FAQ.docx (SEO-нормализация, tier=seo)    │
  │                                                                   │
  │  ТРЕК «Информационное SEO» (блог) - независим от трека «Сайт»:    │
...
Главное правило: **рабочие скилы пишут только в свою папку задачи** (`articles/NNN/`, `strategies/NNN/`, `sites/NNN/`, `structures/NNN/`, `metatags/NNN/`, `audits/NNN/`, `texts/NNN/`, `faq/NNN/`, `topics/NNN/`) + `.claude/tmp/` + `.claude/handoff-requests/`. Общие файлы (`ЗАКАЗЧИК.md`, `template.html`, `topics.xlsx`, весь `.claude/` кроме tmp/handoff-requests) - read-only в worktree, их защищает pre-commit hook.
...
`/seo-tehaudit <domain>`. Самодостаточен: нужны только домен и доступы клиента к Яндекс Вебмастеру и Метрике (`ЗАКАЗЧИК.md` не требуется; регион и база Keyso опционально из анализа `/site-analiz`). На выходе - `A12.md` + `A12.docx`: проблемы по приоритетам (🔴🟡🟢), чеклист для разработчика, динамические приложения. Ни от чего не зависит, результат - дев-команде клиента.

**Трек «Сайт» - коммерческие страницы сайта.**
`/site-analiz` -> (`/seo-struktura` -> `/seo-metategi`, только при SEO) -> `/site-tekst` (тексты + прототип) -> `/seo-faq` (SEO-нормализация готовых страниц, только при SEO). Работает от брифа клиента, **НЕ требует** `/seo-shablon`. `/site-analiz` - единственный вход: структура и тексты читают его контракт `sites/NNN/project.json` и только после гейта заказчика (`queue.json` -> `gate.approved`). Связи ВНУТРИ трека: `/seo-struktura <NNN>` читает `project.json` сама и требует tier=seo; `/site-tekst --site <NNN>` импортирует факты, ЦА, пожелания и затравку конкурентов скриптом, а состав страниц берет из `structure_data.json` (структуры `--structure <MMM>` или анализа); `/seo-faq --from-tekst` берет готовые тексты (v9 и старые v7) или живые URL.

**С SEO** (tier=seo): `/site-analiz` -> `/seo-struktura <NNN>` (SEO-база, мастер-список, A6.xlsx, гейт заказчика) -> `/site-tekst --site <NNN> --structure <MMM>`; параллельно `/seo-metategi --from-structure <MMM>`, после текстов - `/seo-faq --from-tekst <KKK>`.

**Без SEO** (tier=basic - сайты партнера-разработчика, лендинги): `/site-analiz` (состав страниц пишет сам анализ - `sites/NNN/structure_data.json`, решение d9 документа 1) -> `/site-tekst --site <NNN>`. Структуры и FAQ нет - SEO не куплено. Докупили SEO позже - `queue.mjs init <slug> --tier seo`, затем `/seo-struktura <NNN>`.

**Трек «Информационное SEO» - блог/статьи. ПОЛНОСТЬЮ независим от трека «Сайт».**
`/seo-shablon` -> `/seo-temi` -> `/seo-statya`. `/seo-shablon` дает `ЗАКАЗЧИК.md` (профиль) + `template.html` (шаблон статьи); их читают `/seo-temi` и `/seo-statya`. Трек самодостаточен и с треком «Сайт» **не пересекается**.

Заметки:
- Направления не зависят друг от друга - бери любое, какое заказал клиент, в любом порядке (или несколько параллельно).
- Порядок важен только ВНУТРИ трека: сайт - анализ (с гейтом) перед структурой и текстами, структура (при SEO) перед текстами; информационный - шаблон перед темами/статьями.
...
### /site-analiz [<слаг|каталог>] [--tier basic|seo] [--type services|shop|both] [--kind landing|multipage] [--answers <файл>]
- **Зона:** worktree клиента. Предпродажный: `ЗАКАЗЧИК.md` не нужен.
- **Вход:** вводная - бриф, созвон, документы, старый сайт (файлы в `sites/NNN-slug/input/`); пустой вход законен - тогда три вопроса про бизнес одним сообщением. **Для ВСЕХ клиентов**: три вопроса ОПЕРАТОРУ (не заказчику) - SEO куплено? (`tier` seo | basic), услуги или магазин? (`type`), лендинг или многостраничник? (`kind`); флагами `--tier`, `--type`, `--kind` можно ответить сразу.
- **Шаги:** 0 `queue.mjs init` -> 1 фактура (`site-intake`: `parts/facts.json` с источником у каждой строки, `parts/facts-src.json` - дословные цитаты, живут весь срок задачи) -> 2 смыслы и разведка (`site-market`: 2-4 сегмента, профиль ниши, оффер, 3-5 главных лидеров) -> 3 контракт (`build-project.mjs` + `verify-data.mjs`: схема, бюджет знаков, дословность цитат, служебные пометки в значении факта) -> 3b состав страниц без SEO (`pages-planner`, только basic + многостраничник; лендингу одну главную пишет сборка) -> 4 два документа (`build-doc.mjs`) и Drive -> 5 гейт ответов.
- **Вопросы/паузы:** гейт заказчика - документ 1 «Что мы поняли о вашем бизнесе» (решения d1-d9 с дефолтом, d9 - состав сайта при basic) и документ 2 «Что нам от вас нужно» (не больше 10 вопросов). Ответы - лист `answers.txt` -> `apply-answers.mjs` (дифф-лист до записи) -> `queue.mjs gate --by "<кто>"`. Согласование ставит только человек.
- **Выход:** `sites/NNN-slug/`: `queue.json` (tier, гейт, журнал исключений), `project.json` (контракт), `parts/`, `structure_data.json` (basic), `docs/understood.html`, `docs/ask.html`, `answers.txt`.
- **Дальше читает:** tier seo - `/seo-struktura <NNN>` (`project.json`), затем `/site-tekst --site <NNN> --structure <MMM>`; tier basic - `/site-tekst --site <NNN>` (состав - `sites/NNN/structure_data.json`). `/seo-tehaudit --from-analysis <NNN>` берет регион и базу Keyso. Структура и тексты стартуют только после гейта.
- **Агенты:** `site-intake`, `site-market`, `pages-planner` (шаг 3b). Скрипты: `.claude/scripts/site/` (`queue`, `build-project`, `verify-data`, `build-doc`, `apply-answers`, `_contract`).

### /seo-struktura <NNN> [--resume] [--review | --auto] [--import <xlsx>] [--metatags deep|bulk|none]
- **Зона:** worktree клиента. **Требует** анализ `sites/NNN-*/project.json` с tier=seo в `queue.json` (проверяет `validate-project-input.mjs` до первого MCP-вызова; анализа нет - стоп с подсказкой `/site-analiz`; tier basic - стоп: без SEO состав страниц пишет планировщик анализа, докупили SEO - `queue.mjs init <slug> --tier seo`). Гейт анализа не пройден - предупреждение.
- **Вход:** NNN - номер папки анализа `sites/NNN-*/`. Первым внутренним шагом агент `seo-base` делает SEO-базу: база Keyso, конкуренты с метриками, SERP-вердикт, стоп-лист (в папке структуры).
...
- **Дальше читает:** `structure_data.json` - `/site-tekst --structure <MMM>` и `/seo-metategi --from-structure <MMM>`; `A6.md` - вход для ТЗ верстальщика (У5) и далее.
- **Агенты:** `seo-base`, `master-list-builder`, `marker-finder`, `semantic-expander`, `cannibalization-resolver`, `structure-writer`, `structure-verifier`.
...
### /site-tekst --site <NNN> [--structure <MMM>] [--allow-ungated] [--pilot <slug,...>] | <KKK> --resume | --wave <N> | --fix <slug> "<правка>"
- **Зона:** worktree клиента.
- **Концепция (v9):** алгоритм текстов целиком лежит в `.claude/skills/site-tekst/kit/` (воркфлоу, промты, скрипты, правила, схемы); скил ведет задачу: `task.mjs` (plan, init, place, args, status, preview, find), гейты, resume, сдача. Копия kit кладется в папку задачи при каждом старте и `--resume` - это кеш, в git не идет; проектные правки файлов kit - в `overrides/`.
- **Вход:** `--site <NNN>` - анализ `sites/NNN-*/project.json`, импорт скриптом (факты, ЦА, пожелания d1-d5, профиль ниши, затравка конкурентов) **только после гейта анализа**; `--allow-ungated` - пилот до гейта явным флагом. Состав страниц: `--structure <MMM>` (структура SEO) или `sites/NNN/structure_data.json` (анализ без SEO); нет ни того, ни другого - резервная карта агентом. Запасной вход без анализа - `--doc <google doc id> --slug <slug>`.
- **Вопросы/паузы (гейты человека):** 1 - после фазы 0 (карта страниц и `rules/decisions.md`), 2 - после фазы 4 (стратегия и раскладки, выбор пилота), 3 - пилот, 4 - волна 1. Режима `--auto` нет.
- **Поток:** фаза 0 импорт и факты -> гейт 1 -> фаза 2 конкуренты и разбор лидеров по типам (единственный в конвейере) -> фаза 3 аудит типов -> фаза 4 стратегия, брифы, раскладки -> гейт 2 -> пилот (запись, аудит, сборка) -> гейт 3 -> волна 1 -> гейт 4 -> остальные волны -> фаза 7 каталог (ТЗ) -> фаза 8 сборка прототипа и отчет.
- **Опции:** `--pilot a,b`, `--wave <N>` (одна волна и стоп), `--fix <slug> "<правка>"` (точечная правка готовой страницы по находке человека), `--resume [KKK]` (по `meta.json.state`; задача не v9 - стоп).
- **Выход:** `texts/KKK-slug/`: `meta.json` (`format: "v9"`, state, ссылки на анализ и структуру), `config/project.json`, `rules/decisions.md`, `overrides/`, `inputs/`, `work/` (факты, карта, конкуренты, типы, стратегия, страницы `work/pages/<slug>/page.md`, аудит), `work/output/prototype.html` + `report.md`.
- **Дальше:** правки - `/site-tekst <KKK> --fix <slug> "..."`; SEO-слой - `/seo-faq --from-tekst <KKK>` (только tier seo).
- **Модели:** в `args` воркфлоу (`model` opus, `model_light` sonnet); промты kit - не файлы агентов.
...
- **Вход:** `--from-tekst <NNN>` (страницы из `texts/NNN-*/`: задача v9 - `work/sitemap.json` + `work/pages/<slug>/page.md`, бренд, регион, запреты и путь к фактам `read-faq-input.mjs` дописывает в `inputs.json`, `region_yandex` ставит оркестратор; старая задача v7 - текст из page.json + запросы из pages.json; NNN faq зеркалит NNN текстов), `--from-table <путь>` (URL/Маркер/Запросы[/Текст]), `--url <URL>` (одна живая страница - текст спарсит сам). Без источника спросит.
...
- **Опции:** `--resume`; `--no-share` (только локальные `A12.md` + `A12.docx`, без Drive и цикла правок); `--from-analysis <NNN>` (регион и база Keyso из анализа `sites/NNN/project.json`: при SEO - `validate-project-input.mjs`, без SEO - `business.region` и штатный fallback «база по региону»; старые `analyses/NNN/` читаются как раньше; без флага скил ищет анализ с тем же доменом); `--pages <N>` (сколько страниц охватить on-page аудитом, дефолт 24, потолок 80).
...
### /share-structure | /share-metatags | /share-audit | /share-faq | /share-strategy | /share-topics | /share-article  <NNN> [--redo]
- **Зона:** worktree клиента.
...
- **Анализ и тексты:** у `/site-analiz` ссылки на документы в Drive пишет `queue.mjs docs` (перезалить - шаг 4 скила заново), у `/site-tekst` прототип - файл `work/output/prototype.html`, отдельного share-скила у них нет.
...
- **Вопросы/паузы:** предупреждает, если задача - анализ `sites/NNN/` без пройденного гейта заказчика (`queue.json` -> `gate.approved` не `true`). При merge-конфликте останавливается и просит разрешить вручную в main, затем `/handoff --resume` для cleanup.
...
/site-analiz --> sites/NNN/: queue.json (tier, гейт), parts/facts.json + parts/facts-src.json (цитаты),
        |       parts/market.json, project.json (контракт), docs/understood.html + docs/ask.html,
        |       answers.txt -> гейт заказчика (queue.mjs gate)
        |
        +--> tier basic: pages-planner --> sites/NNN/structure_data.json (решение d9)
        +--> tier seo:   /seo-struktura NNN читает project.json (validate-project-input.mjs, тир-гейт)
        v
/seo-struktura --> seo-base (competitors.json, serp.json, stop_list.md) --> A6.xlsx (клиенту)
        |       --> client_filled.xlsx --> structure_data.json --> A6.md
        +--> structure_data.json -> /seo-metategi --from-structure и /site-tekst --structure
        v
/site-tekst --site NNN [--structure MMM] --> texts/KKK/: импорт скриптом после гейта анализа
        |   (work/facts.json, audience.json, client-preferences.json, competitors/seed.json,
        |   inputs/structure_data.json, inputs/analysis.md) -> [гейт 1: карта] -> лидеры, типы,
        |   стратегия, раскладки -> [гейт 2] -> пилот [гейт 3] -> волна 1 [гейт 4] -> волны ->
        |   work/pages/<slug>/page.md -> work/output/prototype.html + report.md
        +--> /seo-faq --from-tekst KKK --> FAQ.docx (SEO-блок поверх готовых страниц; только tier=seo)

БЕЗ SEO (tier=basic) - тот же трек без структуры и FAQ:
бриф --> /site-analiz (состав страниц пишет анализ) --> /site-tekst --site NNN

ТРЕК «Информационное SEO» (самостоятельная цепочка, независим от трека «Сайт»):
...
- `ЗАКАЗЧИК.md` - профиль клиента информационного трека; создается `/seo-shablon` (+ `/handoff-process`), читают `/seo-temi` и `/seo-statya`. Треку «Сайт» и стратегии не обязателен.
- `sites/NNN/project.json` - единственный вход структуры и текстов; читается только после гейта заказчика (`queue.json` -> `gate.approved`). `/seo-struktura` проверяет его `validate-project-input.mjs` (схема, тир-гейт по `queue.json.tier`), `/site-tekst` импортирует скриптом `kit/scripts/import-project.mjs`. Состав страниц один формат - `structure_data.json` (анализ при basic, структура при seo). Старые `analyses/NNN/` (v7) - данные: их читает только `/seo-tehaudit --from-analysis`.
...
| Yandex-выдача | `arsenkin_top` (домены/URL топа: queries[], se=[{"type": 2, "region": <код>}], depth, is_snippet; регион задается ТОЛЬКО внутри se - параметра region на верхнем уровне нет, переданный там код молча игнорируется и выдача приходит московская), `arsenkin_positions` (позиция домена) | запрос + регион -> живая выдача / позиция домена | SERP-анализ, проверка позиций. Альтернативы: keyso `check_top`/`history_serp`; форум-майнинг -> встроенный `web_search` |
| Keys.so | `domain_pages`, `domain_competitors`, `domain_keywords`, `keyword_info`, `keyword_similar`, `domain_dashboard`, `visibility_rating` | домен/ключ + база региона -> страницы, конкуренты, ключи с метриками | SEO-база структуры (`seo-base`: конкуренты с метриками) и маркеры `/seo-struktura`; база домена в `/seo-tehaudit` |
...
| Fetch (seo-fetch) | `seo_fetch_page` (один URL), `seo_fetch_batch` (веер по списку URL) | URL -> разобранный HTML под профиль: `http` (статусы/редиректы/soft-404; для самих редиректов `follow_redirects=false`), `meta` (title/description/canonical/robots/og), `outline` (h1-h6/типизация, дефолт), `content` (основной текст/факты/посылы), `audit` (полный on-page без сырого HTML), `raw` (сырой HTML/CSS - бренд-цвета, шрифты, сигнатуры CMS). robots.txt/sitemap.xml -> `seo_fetch_page(url)` без профиля (тело в body_raw). JS не рендерит | site-market, фаза 2 `/site-tekst` (снимки лидеров), типизация страниц. Деградированный fallback - `web_fetch`/WebFetch (теряет мету/структуру/HTTP-статус) |
| Google Drive | `mcp__gdrive-piotr__uploadFile` (`convertToGoogleFormat: true`), `deleteItem` | локальный .docx/.xlsx -> Google Doc/Sheet + ссылка | финальная заливка в strategy/site-analiz (документы)/structure/topics/article и все `/share-*` |
