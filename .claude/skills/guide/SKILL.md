...
- **Опции:** `--depth deep|bulk` - явная глубина (перекрывает авто-определение по фразам запроса). `deep` (по умолчанию) - по каждой странице анализ выдачи Яндекса + Title через Акварель, писатели параллельно; `bulk` - дёшево, по PLAYBOOK + батч-данные, без выдачи и Акварели. `--resume`.
...
- **Агент:** `faq-builder` (веер по страницам, пачками 6-8).
