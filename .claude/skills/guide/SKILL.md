...
- `/guide <тема>` - показать только нужный раздел. Распознавай тему по ключу:
  - имя команды без слеша: `seo-shablon`, `seo-analiz`, `seo-struktura`, `seo-tehaudit`, `seo-strategiya`, `seo-temi`, `seo-statya`, `fix-article`, `rewrite-section`, `handoff`, `handoff-process`, `request-shared-edit`, `share-*`, `new-project` -> раздел 3 по этой команде.
...
РОДИТЕЛЬ ~/seo-projects/  (git-репо: версионирует только свой .claude/)
  ...
  /status                     -> обзор всех проектов (отдано / в работе / zombie)
        |
...
КЛИЕНТ ~/seo-projects/<slug>/
  ...
  │    /seo-strategiya URL      -> Strategy.docx + Smeta.xlsx (КП)      │
  │                                                               │
  ...
  │    /seo-statya N   -> Article.docx + output.html (статьи) │
...
**Трек А - пресейл (нужна стратегия продвижения).**
`/new-project` -> `/seo-strategiya <URL>`. `/seo-strategiya` сам сканирует сайт и спрашивает нишу/регион (работает даже без `ЗАКАЗЧИК.md`), на выходе - коммерческое предложение клиенту: `SEO_Strategy.docx` (без цен) + `Smeta.xlsx` (с тарифами). Поэтому финальная подсказка `/new-project` зовёт именно `/seo-strategiya` - это by design.

**Трек Б - согласованная работа (клиент принял предложение).** Основной конвейер по порядку:
1. `/seo-shablon <URL>` - профиль `ЗАКАЗЧИК.md` + `template.html`. Фундамент: его читают `/seo-temi` и `/seo-statya`.
2. `/seo-analiz` - предпроектный анализ конкурентов (`A2.md` + `A3.md`).
3. `/seo-struktura <NNN>` - структура сайта на базе анализа (`A6.xlsx` -> клиент -> `A6.md`).
4. `/seo-temi` + `/seo-statya N` - темы и статьи для блога.
...
- `/seo-shablon` нужен до контента (статей/тем), потому что они читают `ЗАКАЗЧИК.md`; для `/seo-analiz` и `/seo-struktura` он не обязателен (анализ работает от брифа).
...
- **Что делает:** обзор всех клиентских проектов по свежести. 3 слоя - отдано в main (услуги со state), в работе в живых worktree (вкл. «готово, но не хендофнуто»), zombie-папки. Статус ПРОИЗВОДНЫЙ из `meta.json` каждой задачи, отдельного трекаемого файла нет.
- **Опции:** `--json`.
- **Дальше:** подсвечивает застрявшее - открыть нужный worktree и `/handoff`. Живой worktree с несмерженными коммитами НЕ удалять (только подсветка, без авто-действий).
...
- **Дальше:** `/handoff` -> в main `/handoff-process` (положит файлы в корень). Артефакты читают: `/seo-strategiya`, `/seo-temi`, `/seo-statya`.
...
- **Дальше читает:** `/seo-struktura` берёт `brief.json` + `competitors.json` + `serp.json` + `leader_scan.json`. Промежуточные `recommendations.json` пригодятся `/seo-strategiya` и `/seo-statya`.
...
- **Опции:** `--resume`; `--no-share` (только локальные `A12.md` + `A12.docx`, без Drive и цикла правок); `--from-analysis <NNN>` (взять базу Keyso из `analyses/NNN/`).
...
- **Параллелизм:** шаги onpage и analytics независимы и идут параллельно; Арсенкин (возраст домена) - только в recon, строго последовательно (ломается при параллельных вызовах).
- **Агенты:** `audit-recon`, `audit-indexing`, `audit-onpage`, `audit-analytics`, `audit-writer`. Рендер A12.md/.docx и verify - скрипты (`render-audit-md`, `build-audit-docx`, `verify-audit`).
...
### /seo-statya <N> [--resume] [--review | --auto] [--with-handoff] [--genre="..."] [--platform=site|external|social]
...
```
/seo-shablon --> ЗАКАЗЧИК.md, template.html
        |              |
        |              +--> /seo-temi (ниша/регион/домен), /seo-statya (профиль+шаблон), /seo-strategiya (если есть)
        v
/seo-analiz --> brief.json, competitors.json, serp.json, leader_scan.json, A2.md, A3.md, recommendations.json
        ...
        +--> recommendations.json -> подсказки для /seo-strategiya и /seo-statya
        v
...
/seo-statya --> articles/NNN/: jm/*.json -> tz.md -> sections/*.md -> article.md -> output.html -> Article.docx
```

Ключевые "точки стыковки":
- `ЗАКАЗЧИК.md` - центральный профиль, читается почти всеми. Создаётся только через `/seo-shablon` + `/handoff-process`.
...
| Команды | все рабочие скилы + `/handoff` | только `/handoff-process` | `/new-project`, `/status` |
