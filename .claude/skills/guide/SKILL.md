...
- `/guide <тема>` - показать только нужный раздел. Распознавай тему по ключу:
  - имя команды без слеша: `seo-shablon`, `seo-analiz`, `seo-struktura`, `seo-strategiya`, `seo-temi`, `seo-statya`, `fix-article`, `rewrite-section`, `handoff`, `handoff-process`, `request-shared-edit`, `share-*`, `new-project` -> раздел 3 по этой команде.
